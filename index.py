...
# 读取yml配置
def getYmlConfig(yaml_file='config_nau.yml'):
...
def getUnSignedTasks(session, apis):
    ...
    res = session.post(
        url='https://{host}/wec-counselor-sign-apps/stu/sign/getStuSignInfosInOneDay'.format(host=apis['host']),
        headers=headers, data=json.dumps({}))
    if len(res.json()['datas']['unSignedTasks']) < 1:   #该行用于平常签到
    #if len(res.json()['datas']['leaveTasks']) < 1: #该行用于请假签到
        log('当前没有未签到任务')
        exit(-1)
    log(res.json())
    latestTask = res.json()['datas']['unSignedTasks'][0] #该行用于平常签到
    #latestTask = res.json()['datas']['leaveTasks'][0] #该行用于请假签到
    return {
...
# Server酱发送通知
def sendMessage(msg, serverchan):
    send = serverchan
    if send != '':
        log('Server酱正在发送通知。。。')
        res = requests.post(url='https://sc.ftqq.com/' + serverchan + '.send',
                            data={'text': msg})
        code = res.json()['errno']
        if code == 0:
            log('Server酱发送通知成功。。。')
